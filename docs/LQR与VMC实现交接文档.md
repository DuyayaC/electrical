...
| LQR | [wheel_leg_lqr.h](../FINAL-2026/User/WheelLeg/wheel_leg_lqr.h)、wheel_leg_lqr.c | 计算 [T, Tp] = K (x_ref - x) |
...
| x = [theta,dtheta,xb,dxb,phi,dphi]^T | WheelLegEstimate.x[6] | 已实现基础版本 |
...
| u = K(L0)(x_d-x) | WheelLeg_LQR_Calculate() | 已实现矩阵运算 |
| T 到左右轮力矩 | 无业务调用、无转向 PD | 未实现 |
| Tp 到左右腿绕中心轴力矩 | 无双腿协调 PD | 未实现 |
| [L_d^l,L_d^r] 到 [F_l,F_r] | 无腿长 PID+前馈 | 未实现 |
| gamma_d - gamma_hat 横滚补偿 | 无 gamma 控制器接口 | 未实现 |
| VMC J^T[F,Tp] | WheelLeg_VMC_Calculate() | 已实现数学模块，尚未接到电机输出 |
| 4 个关节电机力矩发送 | 无 DM 电机业务适配/发送层 | 未实现 |
| 2 个轮电机力矩发送 | 现有代码只有旧四麦轮 CAN_cmd_chassis() | 未实现轮腿版接入 |
| 反馈、限幅、超时、故障门控 | 各数学函数有局部输入检查 | 系统级保护未实现 |

论文文字中，T 是纵向状态反馈产生的驱动轮力矩，转向 PD 输出以相反符号叠加到左右轮力矩；Tp 是纵向状态反馈产生的俯仰/腿轴相关力矩，双腿协调 PD 输出以相反符号叠加到左右腿力矩；腿长控制和横滚补偿共同产生左右腿沿腿方向的虚拟力。最终才将每条腿的 [F_i,Tp_i] 交给 VMC。
...
5. 用 J * [dq1,dq4]^T 得到 dL0_m_s 和 dleg_axis_body_rad_s；
...
- 每侧轮地速度先按论文关系计算，再取左右平均作为 dxb；
- xb 从 0 开始，以 xb += dxb * dt 积分；
- 第一次有效更新时先把估计器初始化到 xb=0；
- 任意一侧轮速、姿态、phi_bc 或五连杆状态无效，整次更新返回 0，并把 estimate 清零。

当前速度公式代码形式为：

    dxb_side = (wheel_output_velocity
                + dphi_bc
                + body_pitch_rate) * wheel_radius
                + L0 * dtheta_side * cos(theta_side)
                + dL0 * sin(theta_side)

其中所有角度是 rad，角速度是 rad/s，长度是 m，速度是 m/s。

当前估计器的边界：

- 没有输出论文图 16 中的航向角速度 psi_hat；
- 没有输出横滚角 gamma_hat；
- 没有输出左右独立的 L0_l/L0_r、theta_l/theta_r 控制量结构，虽然这些数据可从两份 LegKinematicState 取得；
- phi_bc_rad 当前主要用于有效性检查，dphi_bc_rad_s 参与 dxb 公式；其真实机械定义和 CAD 映射还没有在本代码中完成。
...
- 四个关节的零位和正方向；
- 轮半径、轮电机减速比和轮输出速度换算；
- phi_bc 的实际杆件定义及其角速度测量方式；
- 机器人质量、质心位置、腿部质量和转动惯量；
- 轮地纯滚动假设的适用范围；
- LQR 的 A(L0)、B(L0) 与最终 Q/R；
- 左右腿虚拟力和关节力矩的物理正方向。
...
函数内部按 x_ref - x 计算误差。调用者不应再额外取一次负号。T 和 Tp 仍然是图 16 的集中式虚拟控制输出，需要经过辅助环分配后才能送给轮电机和 VMC。
