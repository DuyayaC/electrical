...
//改写开源Mahony算法，使用STM32自带的DSP计算库加速计算，将变量统一为结构体形式
...
static void normalize(float32_t *vec, uint32_t size)
{
    float32_t norm;
    float32_t prod;
...
void MahonyAHRSInit(float Kp, float Ki, float dt, MahonyAHRS *mahony)
{
...
void MahonyAHRSUpdateIMU(MahonyAHRS *mahony)
{
    float recipNorm;
