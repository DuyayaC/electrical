...
static Referee_System_t *referee_instance = NULL;
static osThreadId referee_task_id = NULL;
static volatile uint16_t referee_rx_size = 0;
Referee_System_t referee;
...
{
    referee_instance = ref;
    referee_task_id = task_handle;
}

uint16_t Referee_Get_RxEvent_Size(void)
{
    return referee_rx_size;
}
...
static void Referee_Solve_Single_Packet(Referee_System_t *ref, uint8_t *packet_ptr) {
    ...
    // 旧实现：仅累计 Flag，Referee_Status 可能长期保持 DISABLE。
    // 每成功解析一包，标记裁判系统在线并增加标志位。
    ref->Referee_Status = Referee_Status_ENABLE;
...
//     // 1. 调用滑窗解包逻辑解析当前这波数据
//     Referee_Unpack(ref, ref->Rx_Buffer, Size);
