...
static void trigger_update(M3508_t *m3508)
{
    volatile motor_measure_t *motor = get_trigger_motor_measure_point();

    m3508->motor_current[0] = motor->given_current;
    m3508->motor_ecd[0] = motor->ecd;
    m3508->motor_last_ecd[0] = motor->last_ecd;
    m3508->motor_omega[0] = motor->speed_rpm * 6.0f;
}

void motor_value_updata(void)
{
    trigger_update(&m3508);
    m3508_update(&m3508);
...
static void trigger_update(M2006_t *m2006)
{
    volatile motor_measure_t *motor = get_trigger_motor_measure_point();
